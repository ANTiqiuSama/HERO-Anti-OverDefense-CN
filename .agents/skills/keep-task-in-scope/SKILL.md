...
description: Keep implementation, review, research, planning, and debugging on the requested critical path when considering hashes or caches, freeze files, manifests, repeated audits, version churn, feature flags, migrations, compatibility layers, security hardening, broad test runs, scoring gates, new dependencies, abstractions, or infrastructure. Use for long-running improvement loops such as "continue improving", SOTA, or top-tier publication goals, and when optional process work may delay the requested result; skip routine direct changes with obvious scope. 适用于判断长期优化、冻结审计、加固、兼容、迁移、宽测和新抽象是否过度。
---
...
Test the live uncertainty and reachable blast radius. Start with the smallest check that could falsify the change; broaden for shared contracts, unknown consumers, destructive operations, or high-impact risk. Do not rerun settled checks against unchanged code.
