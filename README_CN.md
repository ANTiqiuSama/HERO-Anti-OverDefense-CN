...
> 🧬 **从 [ARIS](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep) 里剥出来的**(~13.7k★ · HuggingFace Daily Papers #1 · 82 个 skill)。ARIS 对自己产出的每一份工作都跑跨模型审阅。那个审阅器确实好用——但它同时把可观的一部分产出花在了提议"加个没人读的哈希""给一条故意留作散文的规则加 lint""加固一个毫秒级就关闭的竞态"上。**HERO 就是那份"留下价值、去掉税"的契约**,从 ARIS 里泛化出来,任何 coding agent 都能用。

这些形状看起来像是:agent 在优化**不被追责**,而不是**活儿干得好**——这是一个能解释观察的**假说**,不是关于这些模型怎么训出来的定论。但不管病根是什么,形状是真的,而且就是四种——**H**ashing、**E**dge cases、**R**ubrics、**O**verbuild。给它们起了名字,你就能说"刚才犯的是哪一种",而不是空对空地争"是不是有点过了"。

这个仓库 = 一段粘进 agent 配置的短文本 + 一份它要制止的行为案例库。
